...
type OracleIPData struct {
	LastUpdatedTimestamp string           `json:"last_updated_timestamp"`
	Regions              []OracleIPRegion `json:"regions"`
}

type OracleIPRegion struct {
	Region string               `json:"region"`
	CIDRs  []OracleIPRegionCIDR `json:"cidrs"`
}

type OracleIPRegionCIDR struct {
...
func (p *OracleProvider) Generate(out *os.File) error {
	// if err := p.Download(); err != nil {
	// 	return err
	// }

	var data OracleIPData
	...
	for _, region := range data.Regions {
		for _, cidr := range region.CIDRs {
			...
			fmt.Fprintf(out, "\t// provider: %s, cidr: %s, region: %s\n", OracleProviderName, cidr.CIDR, region.Region)
...
func NewOracleProvider(output string) *OracleProvider {
