...
func (p *AWSProvider) Generate(out *os.File) error {
	...
	for _, prefix := range append(data.Prefixes, data.IPv6Prefixes...) {
		var ipPrefix string
		if prefix.IPPrefix != "" {
...
func NewAWSProvider(output string) *AWSProvider {
	return &AWSProvider{
		Provider: &Provider{
			Output: output,
